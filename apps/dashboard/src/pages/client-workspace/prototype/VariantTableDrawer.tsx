...
export function VariantTableDrawer({ creative, onClose }: { creative: ProtoCreative; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end bg-black/40" onClick={onClose}>
      <aside
        className="flex h-full w-full max-w-md flex-col gap-4 overflow-y-auto border-l border-volt-border bg-volt-surface p-6"
...
function DrawerMedia({ creative }: { creative: ProtoCreative }) {
  ...
  if (creative.format === "VIDEO" && source !== null) {
    return (
      <video
        ...
        className="w-full rounded-wk bg-black object-contain"
      />
    );
  }
  return (
    <div className="relative">
      <img src={creative.thumb} alt="" className="w-full rounded-wk object-contain" />
