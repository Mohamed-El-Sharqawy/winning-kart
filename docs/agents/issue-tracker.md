...
- **Branch per issue**: implement on a dedicated branch, never directly on a program or long-running branch: `git checkout -b <type>/<short-name>` (for example `feat/granular-status-foundation`).
- **PR against main**: once the QA gate passes, push the branch and open the pull request that carries the work to main: `git push -u origin <branch>`, then `gh pr create --base main` with `Closes #<n>` in the body so the issue links and auto-closes on merge. Merging waits for owner review (see the delivery workflow in AGENTS.md).
